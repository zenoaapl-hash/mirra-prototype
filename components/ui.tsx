...
import {
  Menu, X, Sparkles, Zap, Gift, Star, RefreshCw,
  ChevronRight, Package, Gem, Clock, Mic, Globe,
} from 'lucide-react';
    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 px-4 py-3 transition-all duration-300">
        <div className="mirra-container">
          <div className="glass rounded-2xl px-5 py-3 flex items-center justify-between transition-all duration-300"
            style={scrolled ? { background:'rgba(8,7,13,0.96)', borderColor:'rgba(196,181,253,0.16)' } : {}}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setOpen(!open)}
                ...
                style={{ color: C.pearl, background: open ? `${C.violet}20` : 'transparent' }}
      ...
      {open && (
        <div className="fixed inset-0 z-40 md:hidden flex flex-col"
          style={{ background: 'rgba(8,7,13,0.97)', backdropFilter: 'blur(24px)' }}>
          ...
          <div className="flex-1 overflow-y-auto px-6 pt-4 pb-10">
              ...
              {links.map(l => (
                <Link key={l.href} href={l.href} onClick={() => setOpen(false)}
                  ...
                  <span className="font-display font-bold text-xl"
                    style={{ color: path === l.href ? C.lavender : C.pearl }}>
                    {l.label}
                  </span>
                  <ChevronRight size={18} style={{ color: path === l.href ? C.lavender : C.subtle }}/>
                </Link>
              ))}
            </div>
            ...
            <Link href="/quiz" onClick={() => setOpen(false)}
              className="flex items-center justify-center gap-2 w-full py-4 rounded-2xl font-display font-bold text-base text-white mb-4"
              style={{ background: `linear-gradient(135deg, ${C.plum}, ${C.violet})`, boxShadow: `0 8px 32px ${C.plum}55` }}>
              <Sparkles size={16}/> Create My MIRRA Self
            </Link>

            {/* Footer links */}
            <div className="flex justify-center gap-6 text-xs" style={{ color: C.subtle }}>
              {[['Privacy','/privacy'],['Terms','/terms']].map(([l,h])=>(
                <Link key={l} href={h} onClick={() => setOpen(false)} className="hover:text-white transition-colors">{l}</Link>
              ))}
            </div>
          </div>
        </div>
    ...
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
      <defs>
        <linearGradient id="mlg" x1="0" y1="0" x2="32" y2="32" gradientUnits="userSpaceOnUse">
          <stop stopColor="#6D28D9"/><stop offset="0.5" stopColor="#8B5CF6"/><stop offset="1" stopColor="#C4B5FD"/>
        </linearGradient>
      </defs>
...
// ─── AVATAR ──────────────────────────────────────────────────────
export function Avatar({ name, size = 48, gradientKey }: { name: string; size?: number; gradientKey?: string }) {
  const initials = name.split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase();
  const grad = gradientKey && ITEM_GRADIENTS[gradientKey] ? ITEM_GRADIENTS[gradientKey] : `linear-gradient(135deg, ${C.plum}, ${C.violet})`;
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute rounded-full flex items-center justify-center" style={{ inset: '3px', background: grad }}>
...
// ─── LARGE AVATAR ────────────────────────────────────────────────
export function LargeAvatar({ name, gradientKey, emoji, size = 80 }: { name: string; gradientKey: string; emoji: string; size?: number }) {
...
export function ItemCard({ name, type, rarity, gradientKey, size = 'md' }: {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden flex-shrink-0 cursor-pointer group transition-all duration-200 hover:scale-105"
      style={{ width: dims, height: dims, border: `2px solid ${rc.border}`, background: rc.bg,
        boxShadow: rarity === 'legendary' ? `0 0 24px ${rc.color}40` : rarity === 'epic' ? `0 0 16px ${rc.color}30` : 'none' }}>
      <div className="absolute inset-0 opacity-30 group-hover:opacity-45 transition-opacity" style={{ background: grad }}/>
      {/* Inner shape */}
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className="rounded-xl flex items-center justify-center"
          style={{ width: dims * 0.42, height: dims * 0.42, background: grad }}>
          <span style={{ fontSize: dims * 0.24, color: 'rgba(255,255,255,0.9)' }}>{emoji}</span>
        </div>
      </div>
      {/* Rarity indicator */}
      ...
      {/* Legendary shimmer line */}
      {rarity === 'legendary' && (
        <div className="absolute inset-0 rounded-2xl pointer-events-none"
          style={{ background: `linear-gradient(45deg, transparent 30%, ${rc.color}15 50%, transparent 70%)` }}/>
      )}
    </div>
  );
}
...
export function SectionHeader({ eyebrow, title, subtitle, gradient = 'violet', align = 'center' }: {
  ...
  gradient?: 'violet' | 'rose' | 'aurora' | 'plum'; align?: 'center' | 'left';
}) {
  return (
    ...
    <div onClick={onClick}
      ...
      <div className="h-28 relative flex items-end p-3" style={{ background: wg.bg }}>
        ...
        <div className="absolute top-2.5 right-2.5 glass px-2 py-0.5 rounded-full font-display font-semibold"
          style={{ color: C.lavender, fontSize: 10 }}>{world.daysLeft}d left</div>
      </div>
    ...
    <div className="flex items-center gap-1.5">
      {[{ v: h, l: 'HRS' }, { v: m, l: 'MIN' }].map(({ v, l }) => (
        <div key={l} className="glass rounded-lg px-2 py-1 text-center min-w-[38px]">
          <div className="font-display font-bold text-sm" style={{ color: C.lavender }}>{String(v).padStart(2,'0')}</div>
...
export function Footer() {
  return (
    <footer className="border-t mt-20" style={{ borderColor: 'rgba(196,181,253,0.08)' }}>
      <div className="mirra-container py-12">
        <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
          <div className="text-center md:text-left">
            ...
            <p className="text-xs max-w-xs leading-relaxed" style={{ color: C.subtle }}>
              Social identity-commerce for Gen Z and young Millennials.<br/>US · UK · Japan
            </p>
          </div>
        ...
        <div className="border-t mt-8 pt-6 text-center text-xs"
          style={{ borderColor:'rgba(196,181,253,0.06)', color:'rgba(63,63,70,0.7)' }}>
